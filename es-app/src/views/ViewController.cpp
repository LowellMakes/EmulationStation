...
bool ViewController::input(InputConfig* config, Input input)
{
	...
	// open menu
	if(!(!UIModeController::getInstance()->isUIModeFull() && Settings::getInstance()->getBool("DisableKidStartMenu")) && config->isMappedTo("start", input) && input.value != 0)
	{
		// open menu
...
std::vector<HelpPrompt> ViewController::getHelpPrompts()
{
	...
	if(!(!UIModeController::getInstance()->isUIModeFull() && Settings::getInstance()->getBool("DisableKidStartMenu")))
